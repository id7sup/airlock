...
export default async function PublicSharePage({ 
  ...
  if ((result as any).error) {
    ...
    // Récupérer le propriétaire pour notifier de l'expiration/quota
  ...
  // 2. Récupérer le propriétaire pour les notifications de succès
  const ownerPerm = await db.collection("permissions")
  ...
  // 3. Incrémenter le nombre de vues via analytics
  ...
  if (link.passwordHash) {
    ...
    if (!pwd || !isPasswordCorrect) {
      ...
      // Notification d'accès réussi via mot de passe
      if (ownerId) {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-apple-secondary text-sm font-semibold uppercase tracking-widest mt-1 opacity-50">
                  {link.folder.files.length} fichiers • Partage Sécurisé
                </p>
              </div>
            </div>
          </div>
